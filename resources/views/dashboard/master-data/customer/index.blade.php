...
@section('main')
  <div>
        ...
        <nav class="mb-5 flex" aria-label="Breadcrumb">
          <ol class="inline-flex items-center space-x-1 text-sm font-medium md:space-x-2">
            ...
            <li>
              <div class="flex items-center">
                <x-fas-chevron-right class="h-3 w-3 text-gray-400" />
                <span class="ml-1 text-gray-400 md:ml-2" aria-current="page">Pelanggan</span>
              </div>
            </li>
          </ol>
        </nav>
        <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl">Pelanggan</h1>
      </div>
    </div>
    ...
    <div class="p-4 bg-white rounded-lg shadow-lg 2xl:col-span-2 sm:p-6 mb-4">
      <div class="block items-center justify-between sm:flex md:divide-x md:divide-gray-100 mb-4">
        <div class="mb-4 flex items-center sm:mb-0">
          <form class="sm:pr-3" action="{{ route('dashboard.master-data.customer') }}" method="GET">
            <label for="supliers-search" class="sr-only">Search</label>
            <div class="relative mt-1 w-48 sm:w-64 xl:w-96">
              <input type="text" name="search" id="supliers-search"
                ...
                placeholder="Cari Pelanggan" @if (isset($_GET['search'])) value="{{ $_GET['search'] }}" @endif>
            </div>
          </form>
        ...
        {{-- @can('master data-produk-tambah') --}}
          <a id="createPelangganButton"
            ...
            href="{{ route('dashboard.master-data.customer.create') }}">
            Tambahkan Pelanggan
          </a>
        {{-- @endcan --}}
      </div>
            ...
            <div class="overflow-hidden shadow rounded-t-lg">
              <table class="min-w-full table-fixed divide-y divide-gray-200">
                ...
                <tbody class="divide-y divide-gray-200 bg-white">
                  @forelse ($data as $item)
                    <tr class="hover:bg-gray-100">
                      ...
                      <td class="whitespace-nowrap p-4 text-sm font-normal text-gray-500">
                        <p class="text-sm font-normal text-gray-900">{{ $item->code }}
                        </p>
                      </td>
                      <td class="whitespace-nowrap p-4 text-sm font-normal text-gray-500">
                      ...
                      <td class="whitespace-nowrap p-4 text-sm font-normal text-gray-500">
                        <p class="text-sm font-normal text-gray-900">{{ $item->address }}
                        </p>
                      </td>
                      <td class="whitespace-nowrap p-4 text-sm font-normal text-gray-500">
                        <p class="text-sm font-normal text-gray-900">{{ $item->phone_number }}
                        </p>
                      </td>
                      <td class="whitespace-nowrap p-4 text-sm font-normal text-gray-500">
                        <p class="text-sm font-normal text-gray-900">{{ $item->email }}
                        </p>
                      </td>
                      <td class="whitespace-nowrap p-4 text-sm font-normal text-gray-500">
                        <p class="text-sm font-normal text-gray-900">{{ $item->bank_account_number }}
                        </p>
                      </td>
                      <td class="whitespace-nowrap p-4 text-sm font-normal text-gray-500">
                      ...
                      <td class="text-center space-x-2 whitespace-nowrap p-4">
                        {{-- @can('master data-produk-perbarui') --}}
                          <a href="{{ route('dashboard.master-data.customer.edit', ['id' => $item->id]) }}"
                            id="updatePelangganButton" data-drawer-target="drawer-update-customer-default"
                            data-drawer-show="drawer-update-customer-default" aria-controls="drawer-update-customer-default"
                        ...
                        {{-- @can('master data-produk-hapus') --}}
                          <button type="button" id="deletePelangganButton"
                            data-drawer-target="drawer-delete-customer-default"
                            data-drawer-show="drawer-delete-customer-default" aria-controls="drawer-delete-customer-default"
                            data-drawer-placement="right"
                  ...
                  @empty
                    <tr>
                      <td class="text-center text-base font-light p-4" colspan="9">Data Kosong</td>
                    </tr>
                  @endforelse
                </tbody>
              </table>
    ...
    <!-- Delete Pelanggan Drawer -->
    <div id="drawer-delete-customer-default"
      ...
      <button type="button" data-drawer-dismiss="drawer-delete-customer-default"
        aria-controls="drawer-delete-customer-default"
      ...
      <form id="form-delete">
        ...
        <button type="button"
          ...
          data-drawer-hide="drawer-delete-customer-default">
          Tidak, Batalkan
        </button>
      </form>
    </div>
  ...
  <script type="text/javascript">
    window.onload = () => {
      document.addEventListener('click', async (event) => {
        // DELETE DATA
        if (event.target.getAttribute('data-drawer-target') == "drawer-delete-customer-default") {
        ...
        if (event.target.getAttribute('data-type') == "button-delete") {
          ...
          document.querySelector("#form-delete").action =
            `/dashboard/master-data/customer/${id}`;
