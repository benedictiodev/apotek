...
@section('main')
  <div>
        ...
        <nav class="mb-5 flex" aria-label="Breadcrumb">
          <ol class="inline-flex items-center space-x-1 text-sm font-medium md:space-x-2">
            ...
            <li>
              <div class="flex items-center">
                <x-fas-chevron-right class="h-3 w-3 text-gray-400" />
                <span class="ml-1 text-gray-400 md:ml-2" aria-current="page">Karyawan</span>
              </div>
            </li>
          </ol>
        </nav>
        <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl">Karyawan</h1>
      </div>
    </div>
    ...
    <div class="p-4 bg-white rounded-lg shadow-lg 2xl:col-span-2 sm:p-6 mb-4">
      <div class="block items-center justify-between sm:flex md:divide-x md:divide-gray-100 mb-4">
        <div class="mb-4 flex items-center sm:mb-0">
          <form class="sm:pr-3" action="{{ route('dashboard.master-data.employee') }}" method="GET">
            ...
            <div class="relative mt-1 w-48 sm:w-64 xl:w-96">
              <input type="text" name="search" id="supliers-search"
                ...
                placeholder="Cari Karyawan" @if (isset($_GET['search'])) value="{{ $_GET['search'] }}" @endif>
            </div>
          </form>
        ...
        {{-- @can('master data-produk-tambah') --}}
          <a id="createKaryawanButton"
            ...
            href="{{ route('dashboard.master-data.employee.create') }}">
            Tambahkan Karyawan
          </a>
        {{-- @endcan --}}
      </div>
                    ...
                    <tr class="hover:bg-gray-100">
                      ...
                      <td class="text-center space-x-2 whitespace-nowrap p-4">
                        {{-- @can('master data-produk-perbarui') --}}
                          <a href="{{ route('dashboard.master-data.employee.edit', ['id' => $item->id]) }}"
                            id="updateKaryawanButton" data-drawer-target="drawer-update-employee-default"
                            data-drawer-show="drawer-update-employee-default" aria-controls="drawer-update-employee-default"
                        ...
                        {{-- @can('master data-produk-hapus') --}}
                          <button type="button" id="deleteKaryawanButton"
                            data-drawer-target="drawer-delete-employee-default"
                            data-drawer-show="drawer-delete-employee-default" aria-controls="drawer-delete-employee-default"
                            data-drawer-placement="right"
    ...
    <!-- Delete Karyawan Drawer -->
    <div id="drawer-delete-employee-default"
      ...
      <button type="button" data-drawer-dismiss="drawer-delete-employee-default"
        aria-controls="drawer-delete-employee-default"
      ...
      <form id="form-delete">
        ...
        <button type="button"
          ...
          data-drawer-hide="drawer-delete-employee-default">
          Tidak, Batalkan
        </button>
      </form>
    </div>
  ...
  <script type="text/javascript">
    window.onload = () => {
      document.addEventListener('click', async (event) => {
        // DELETE DATA
        if (event.target.getAttribute('data-drawer-target') == "drawer-delete-employee-default") {
        ...
        if (event.target.getAttribute('data-type') == "button-delete") {
          ...
          document.querySelector("#form-delete").action =
            `/dashboard/master-data/employee/${id}`;
